...
public interface MenuRequestRepository extends JpaRepository<MenuRequest, Long> {
    ...
    Page<MenuRequest> findAllByStateAndAcceptedDate(MenuState state, LocalDateTime date, PageRequest pageRequest);

    @Query("SELECT menu FROM MenuRequest menu WHERE menu.state=:state ORDER BY menu.createAt")
    Page<MenuRequest> findAllByState(MenuState state, PageRequest pageRequest);
}
